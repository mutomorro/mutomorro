...
// Live usage (`<OrgDesignHero />`) passes no props and behaves exactly as
...
export default function OrgDesignHero({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      cx = W / 2
      cy = H / 2
      // Seed only the one-time layout init, then restore the real RNG. The
      // layout is rebuilt on every resize, so seed/restore wraps each rebuild.
