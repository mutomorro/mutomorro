...
- **Deliverability pass - DONE** (`deliverability-audit-2026-06-25.md`). Auth is sound (SPF aligned via `send.mutomorro.com`, DKIM `d=mutomorro.com` aligned, **DMARC PASSES**); the photekton spam landing was a thin-reputation + dead Gmail catch-all artefact, not a systemic fault. **No hard blocker for the full-list send.** Pre-send: check Resend Deliverability Insights + Google Postmaster Tools; confirm the 3,800 is engaged (consider seeding the most-engaged segment first). Later hardening: DKIM 1024→2048, DMARC p=none→p=quarantine.
