...
export const RESOLVE = {
  tool: { sanityType: 'tool', fields: { overview: 'heroImage' } },
...
// ── Staged rollout ───────────────────────────────────────────────────────────
// Only enabled docs EMIT proxy URLs (the route itself resolves any slug). PILOT:
// process-mapping — a reset-cohort, low-traffic tool. Rollout = add an entry.
export const ENABLED_SLUGS = new Set(['tool:process-mapping'])
...
export function renderSrcSet(typeSegment, slug, widths, fmt) {
  return widths
    .map((w) => `${withParams(overviewPath(typeSegment, slug), { fmt, w })} ${w}w`)
    .join(', ')
}
