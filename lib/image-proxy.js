...
export const RESOLVE = {
  ...
  // `stagesField`/`stageImageField` — service `stages[]` infographics. Each stage OBJECT carries
  // a permanent `stage-N-name` imageSlug; the image is the stage's `stageImage` sub-field. The
  // route resolves a `stage-*` role through `stages[imageSlug == $s][0].stageImage` (not a body[]
  // block). Canonical URL is the FLAT shape `/img/service/<slug>-stage-N-name` (same as a tool's
  // step-N), so render + sitemap reuse the flat builders below.
  service: { sanityType: 'service', stagesField: 'stages', stageImageField: 'stageImage', fields: { overview: 'heroImage', proposition: 'propositionImage' } },
...
// Service stage infographic (render gate + sitemap). The flat proxy URL when the service is
// enabled AND the stage has a backfilled `imageSlug` (= a `stage-N-name` role the route can
// resolve), else the raw CDN url unchanged. The imageSlug gate is what makes shipping the code
// inert until the backfill runs: a stage with no slug keeps emitting its CDN url, so the page
// and sitemap never advertise a proxy URL the route would 404. `imageSlug` IS the flat URL's
// role tail, so this reuses the same builder as a tool's step-N body image.
export function stageImageProxyUrl(typeSegment, slug, imageSlug, fallbackCdnUrl, { absolute = false } = {}) {
  if (!isProxyEnabled(typeSegment, slug) || !imageSlug || !fallbackCdnUrl) return fallbackCdnUrl
  const path = bodyPathBySlug(typeSegment, slug, imageSlug) // flat: `/img/<type>/<slug>-<imageSlug>`
  return absolute ? SITE_URL + path : path
}
