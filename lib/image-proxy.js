...
//   - `overview`              the main / hero diagram (also the consolidated body master)
//   - `proposition`           a second fixed single-field role (service propositionImage)
//   - `step-N-<name>`         a body diagram addressed by its permanent imageSlug
//   - `stage-N-<name>`        reserved variant form (not seeded by the current backfill)
// The named tail is what lets a body image (`step-1-assumed`) share the hero's flat path
// shape. The role is split off the END, so a slug that contains hyphens is safe.
const ROLE_SUFFIX = /^(.+)-(overview|proposition|(?:step|stage)-\d+(?:-[a-z0-9]+)*)$/
...
// on wholesale.
export const ENABLED_TYPES = new Set(['training', 'develop', 'resources', 'sectors', 'service'])
...
// Clean, house-pattern body path — FLAT `/img/<type>/<slug>-<imageSlug>` (same shape as
// the hero; the `/body/` segment is gone). `imageSlug` is a permanent, controlled role
// word (`overview` | `step-N-name`) stored on the image, decoupled from both alt and the
// asset. With no `_key` in the URL the slug is LOAD-BEARING: the route resolves purely by
// it, so an imageSlug is set once and never changed. For the master diagram
// `imageSlug === 'overview'`, so this collapses onto the hero URL (same asset) — a
// deliberate consolidation, not a clash.
export function bodyPathBySlug(typeSegment, slug, imageSlug) {
  return `${PROXY_BASE}/${typeSegment}/${slug}-${imageSlug}`
}
