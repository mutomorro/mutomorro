...
// Body-image canonical PNG (param-less).
export function bodyCanonicalUrl(typeSegment, slug, alt, key, { absolute = false } = {}) {
  const path = bodyPath(typeSegment, slug, alt, key)
  return absolute ? SITE_URL + path : path
}
...
export function bodyRenderSrcSet(typeSegment, slug, alt, key, widths, fmt) {
  return buildSrcSet(bodyPath(typeSegment, slug, alt, key), widths, fmt)
}
...
// Body image for the sitemap (gated).
export function bodySitemapImage(typeSegment, slug, alt, key, fallbackUrl) {
  return isProxyEnabled(typeSegment, slug) ? bodyCanonicalUrl(typeSegment, slug, alt, key, { absolute: true }) : fallbackUrl
}
