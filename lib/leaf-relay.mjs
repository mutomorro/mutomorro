 ...
 * documents. It holds the Leaf bypass credential server-side and forwards each request
...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        content: {
          description:
            'The ProseMirror/TipTap document tree (a doc-topped node, the editor.getJSON() shape). Omit for a blank document.',
        },
        title: { type: 'string' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        id: { type: 'string' },
        content: { description: 'The new ProseMirror/TipTap document tree (doc-topped).' },
        title: { type: 'string' },
        project_id: { type: ['string', 'null'] },
        footer_style: { type: 'string', enum: ['tagline', 'address'] },
      },
