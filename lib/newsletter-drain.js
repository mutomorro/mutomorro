 ...
 *
 * Sentinel sends (issue_key 'zz%') are EXCLUDED: the load-test harnesses
 * (scripts/test-*-drain*.js, test-receipt-storm.js) create real 'draining' rows
 * with synthetic 'zz…@test.invalid' recipients and drive drainOnce(id) directly.
 * Without this guard the every-minute cron also picks them up and REAL-sends them
 * via Resend to those invalid addresses (a 29-Jun-2026 incident: ~10k bounce
 * attempts to @test.invalid). The guard is NULL-safe — a real send with a null
 * issue_key is still drained (real sends never use a 'zz' prefix). */
export async function findActiveDrainingSend(supabase) {
  const { data } = await supabase
    .from('newsletter_sends')
    .select('id, issue_key, subject, content_json, status')
    .eq('status', 'draining')
    .or('issue_key.is.null,issue_key.not.like.zz*')
...
export async function drainOnce(sendId, opts = {}) {
  ...
  const { data: send } = await supabase
    .from('newsletter_sends')
    .select('id, issue_key, subject, content_json, status, calendar_item_id')
    .eq('id', sendId)
    .maybeSingle()
...
async function finalize(supabase, resend, send) {
  ...
  await supabase.from('newsletter_sends')
    .update({ status: 'complete', total_sent: count || 0, completed_at: completedAt })
    .eq('id', send.id)

  // Mark the content-calendar item 'published' (editorial sends only — promo
  // sends carry no calendar_item_id). This is the paced equivalent of the legacy
  // runSend's step 14: the drain only knows the send row, so create-queue stamps
  // calendar_item_id onto it and we resolve it here. Idempotent — a remainder
  // re-POST that finalizes again just re-publishes an already-published item.
  if (send.calendar_item_id) {
    const { error: calErr } = await supabase.from('calendar_items')
      .update({ status: 'published', completed_at: completedAt, updated_at: completedAt })
      .eq('id', send.calendar_item_id)
    if (calErr) console.error('drain: calendar publish failed:', calErr?.message || calErr)
  }
