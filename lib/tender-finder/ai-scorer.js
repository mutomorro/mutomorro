...
- "Primary care transformation" or "ICS transformation strategy" = NHS clinical/operational restructuring = score 1-2, not OD consultancy even though it says "transformation"
- An article titled "Our Fourth Year of the Leadership Development Program" = someone writing about their own programme, not a tender = score 1
...
JOB ADVERT CHECK: Score 1 if this is a job vacancy, employment opportunity, or recruitment listing rather than a procurement/consultancy tender. Job adverts are for hiring an employee - tenders are for commissioning external work. Look for clues: job titles like "Manager", "Consultant", "Officer", "Director" followed by a location and employer; salary bands; "apply by" with a CV/application form; sources like Civil Service Jobs, JobLeads, Indeed, LinkedIn, NHS Jobs, Guardian Jobs, CharityJob, S1Jobs, myjobscotland. Even if the role is doing OD work, James cannot bid on a job - he bids on contracts.

OPPORTUNITY CHECK: Only score highly if this appears to be a live, open opportunity that James could bid on. If the text describes a contract that has already been awarded, a news article or blog post about work someone else is doing or has done, a press release about an appointment or programme launch, a case study or "year in review" of an existing programme, a grant programme James wouldn't qualify for, or a report/announcement about something that has already happened, score 1. News articles that mention "consultants appointed" or describe an organisation's transformation journey are not tenders - they are coverage of work that is already underway or complete. Look for clues like "awarded", "secures", "appoints", "wins contract", or past-tense language describing completed work.
...
export function shouldAiScore(tender) {
  ...
  // Google Alerts: only AI score if keywords matched or it's a trigger event.
  // Zero-keyword alerts are overwhelmingly noise (387/387 scored 1-3 on 2 Apr).
  if (tender.source === 'google-alerts') {
    return tender.keyword_score > 0 || tender.notice_type === 'trigger_event'
  }
 ...
 */
export async function aiScoreBatch(tenders, options = {}) {
  const { timeBudgetMs, startTime } = options

  if (!process.env.ANTHROPIC_API_KEY) {
    console.log('  ANTHROPIC_API_KEY not set - skipping AI scoring')
    return { tenders, aiCalls: 0, aiErrors: 0, stoppedEarly: false, skippedByBudget: 0 }
  }
  ...
  for (const tender of tenders) {
    ...
    if (timeBudgetMs && startTime) {
      const elapsed = Date.now() - startTime
      if (elapsed >= timeBudgetMs) {
        const remaining = tenders.length - aiCalls - aiErrors - skippedByBudget
        skippedByBudget = remaining
        stoppedEarly = true
        console.log(`    Time budget reached (${Math.round(elapsed / 1000)}s) - ${remaining} tenders deferred to tomorrow`)
        break
      }
    }
  ...
  return { tenders, aiCalls, aiErrors, stoppedEarly, skippedByBudget }
}
