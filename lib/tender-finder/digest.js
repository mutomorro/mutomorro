 ...
 *
 * Builds a clean HTML email summarising hot/warm/trigger tenders
...
function buildDigestHtml(content, pipelineSummary) {
  const { hot, warm, triggers } = content
  ...
  if (warm.length > 0) {
    sections += `
      <tr><td style="padding:24px 0 12px;">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr><td style="font-size:18px;font-weight:bold;color:#D97706;padding-bottom:4px;">🟡 WARM OPPORTUNITIES (${warm.length})</td></tr>
          <tr><td style="border-bottom:2px solid #D97706;"></td></tr>
        </table>
      </td></tr>
      <tr><td style="padding-top:16px;">
        ${warm.map(t => tenderRow(t, true)).join('')}
      </td></tr>`
  }
  ...
  // No tenders at all
  if (hot.length === 0 && warm.length === 0 && triggers.length === 0) {
    sections += `
      <tr><td style="padding:24px 0;color:#6B7280;font-size:15px;">
        No hot, warm, or trigger tenders found today. ${stats.cool + stats.archived} lower-scoring results stored for reference.
      </td></tr>`
  }
...
export async function sendDigest(digestContent, pipelineSummary) {
  ...
  const subject = `Mutomorro Tender Digest - ${formatDate(new Date())} - ${stats.hot} hot, ${stats.warm} warm`
