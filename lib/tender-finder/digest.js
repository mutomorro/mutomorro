...
export async function buildDigest(sinceDate) {
  ...
  // Paginated - daily window is small today but tenders table is unbounded.
  let all
  try {
    all = await fetchAllPaginated((from, to) => supabase
      .from('tenders')
      .select('*')
    ...
    throw new Error(`Failed to load tenders for digest: ${err.message}`)
  }
...
/**
 * Heartbeat alert: the scoring stage ran but every AI call failed (e.g. a retired
 * model id returning 404). Without this, a broken scorer is indistinguishable from
 * a quiet day - the digest just silently skips. This email makes the failure
 * visible. Sent only when there were tenders to score and none succeeded.
 */
export async function sendScoringHealthAlert(summary) {
  const resend = new Resend(process.env.RESEND_API_KEY)
  const subject = `Mutomorro Tender Finder - SCORING FAILED (${formatDate(new Date())})`
  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:560px;color:#111827;">
      <h2 style="color:#B91C1C;font-size:18px;">Tender scoring produced no scores</h2>
      <p>The daily AI-scoring stage ran but every call failed, so no tenders were
      scored and today's digest will be empty regardless of whether good
      opportunities came in. This is a real failure, not a quiet day.</p>
      <table cellpadding="0" cellspacing="0" style="font-size:14px;margin:12px 0;">
        <tr><td style="padding:2px 12px 2px 0;color:#6B7280;">Tenders to score</td><td>${summary.attempted ?? '?'}</td></tr>
        <tr><td style="padding:2px 12px 2px 0;color:#6B7280;">Succeeded</td><td>${summary.succeeded ?? 0}</td></tr>
        <tr><td style="padding:2px 12px 2px 0;color:#6B7280;">Errors</td><td>${summary.errors ?? '?'}</td></tr>
      </table>
      <p style="font-size:14px;">Most likely cause: the Claude model id no longer
      resolves, or the Anthropic API is failing. To diagnose:</p>
      <ul style="font-size:14px;color:#374151;">
        <li><code>node docs/stack-checks/check-anthropic-model.mjs</code> - is the scoring model still valid?</li>
        <li>Vercel logs for <code>/api/cron/tender-finder-score</code> - the per-item error message.</li>
      </ul>
    </div>`
  ...
  if (error) throw new Error(`Resend scoring-alert error: ${error.message}`)

  console.log(`📧 Scoring health alert sent: ${subject}`)
}
