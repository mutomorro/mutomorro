...
const DELAY_MS = 200 // Short pause between batches (was 500)
const BATCH_SIZE = 50 // Fetch feeds in parallel batches of 50 (was 10)
const DB_BATCH_SIZE = 50 // Update Supabase in parallel batches of 50
...
async function fetchFeed(feed, retries = 2) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const res = await fetch(feed.rss_url, { signal: controller.signal })
...
/**
 * Batch-update feed polling timestamps in Supabase.
 * Fires updates in parallel chunks instead of one-by-one.
 */
async function batchUpdateFeeds(supabase, feedUpdates) {
  for (let i = 0; i < feedUpdates.length; i += DB_BATCH_SIZE) {
    const batch = feedUpdates.slice(i, i + DB_BATCH_SIZE)
...
export async function searchGoogleAlerts() {
  ...
  console.log(`  ${feeds.length} active feeds to check (batches of ${BATCH_SIZE})`)
  ...
  // Batch-update feed polling timestamps in Supabase (parallel, not sequential)
  await batchUpdateFeeds(supabase, feedUpdates)
