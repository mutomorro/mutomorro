 ...
 *   Stage 3 (runDigestStage)  - Build and send the daily digest + hot alerts
 *
...
import { sendHotAlerts } from './alerts.js'
...
export async function runAiScoreStage(options = {}) {
  ...
  // Find unscored tenders from the last 24 hours
  ...
  console.log(`Found ${summary.found} unscored tenders from the last 24h`)
  ...
  // AI score in batch
  console.log(`\nAI scoring ${unscored.length} tenders...`)
  const aiResult = await aiScoreBatch(unscored)
...
// ═══════════════════════════════════════════════════════
// STAGE 3: Daily digest + hot alerts
// ═══════════════════════════════════════════════════════

/**
 * Stage 3: Build and send the daily digest email and any hot alerts.
 */
export async function runDigestStage() {
  ...
  // Hot alerts
  console.log('\nChecking for hot alerts...')
  try {
    const { data: hotTenders } = await supabase
      .from('tenders')
      .select('*')
      .eq('temperature', 'hot')
      .is('notified_at', null)
    ...
    if (hotTenders?.length > 0) {
      const alertCount = await sendHotAlerts(hotTenders)
      summary.hotAlerts = alertCount
      console.log(`  ${alertCount} hot alert(s) sent`)
    } else {
      console.log('  No hot tenders to alert')
    }
  } catch (err) {
    console.error(`  Hot alerts error: ${err.message}`)
  }
  ...
  try {
    ...
    if (digestContent.stats.total > 0 || digestContent.stats.hot > 0 || digestContent.stats.warm > 0) {
      await sendDigest(digestContent, { stage: 'digest' })
      summary.digestSent = true
      console.log('  Digest sent')
    } else {
      console.log('  No tenders today - digest skipped')
    }
  } catch (err) {
 ...
 * @param {boolean} [options.sendHotAlerts=false] - Send immediate emails for hot tenders
 * @param {boolean} [options.sendDigest=false] - Send daily digest email
...
export async function runPipeline(options = {}) {
  const {
    ...
    dryRun = false,
    skipAi = false,
    sendHotAlerts: doSendAlerts = false,
  ...
  // Hot alerts - immediate emails for 60+ score tenders
  if (doSendAlerts) {
    console.log('\nStep 7a: Hot alerts...')
    try {
      // Re-fetch stored tenders to get their Supabase IDs for notified_at tracking
      ...
      if (storedRows?.length > 0) {
        const alertCount = await sendHotAlerts(storedRows)
        summary.notifications.hotAlerts = alertCount
        console.log(`  ${alertCount} hot alert(s) sent`)
      } else {
        console.log('  No hot tenders to alert')
      }
  ...
  if (summary.notifications?.hotAlerts > 0) console.log(`🔴 Hot alerts sent: ${summary.notifications.hotAlerts}`)
