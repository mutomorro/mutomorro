...
import { buildDigest, sendDigest, sendScoringHealthAlert } from './digest.js'
...
export async function runAiScoreStage(options = {}) {
  ...
  summary.attempted = aiResult.aiCalls
  summary.succeeded = aiResult.aiCalls - aiResult.aiErrors
  ...
  // Heartbeat: if we attempted to score tenders but NONE succeeded, the scorer is
  // failing silently (e.g. a retired model id returning 404). Alert so it can't
  // hide behind an empty digest. attempted === 0 means nothing qualified for AI
  // scoring - a genuine quiet day, no alert.
  if (summary.attempted > 0 && summary.succeeded === 0) {
    console.error(`  SCORING ALERT: attempted ${summary.attempted}, 0 succeeded (${aiResult.aiErrors} errors) - scorer appears broken`)
    try {
      await sendScoringHealthAlert(summary)
    } catch (e) {
      console.error(`  Failed to send scoring health alert: ${e.message}`)
    }
  }
