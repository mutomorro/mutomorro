...
export function recalculateWithAi(tender) {
  ...
  let total = tender.ai_score * 10

  // Small bonuses (max +10 total from keywords/sector/value)
  if (tender.keyword_score > 0) total += Math.min(tender.keyword_score, 5)
  if (tender.sector_score >= 10) total += 3
  if (tender.value_score > 0) total += Math.min(tender.value_score, 2)
