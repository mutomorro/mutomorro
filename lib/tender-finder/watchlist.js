...
export async function checkWatchlist() {
  ...
  for (const entry of entries) {
    console.log(`  Checking: ${entry.organisation}...`)

    try {
      const { text, hash } = await fetchPage(entry.url)
      ...
      // Store snapshot
      await supabase
        .from('tender_watchlist_snapshots')
        .insert({
          watchlist_id: entry.id,
          content_hash: hash,
          content_text: text.slice(0, 50000), // Cap storage
        })
      ...
      if (anthropic) {
        try {
          const prompt = buildWatchlistPrompt(entry.organisation, text)
